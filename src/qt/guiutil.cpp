...
{
    // Convert kekcoin:// to kekcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("kekcoin://"))
    {
        uri.replace(0, 12, "kekcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "KekCoin.lnk";
}
...
{
    return GetAutostartDir() / "kekcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("KekCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  kekcoin-qt [" + tr("command-line options") + "]                     " + "\n";
